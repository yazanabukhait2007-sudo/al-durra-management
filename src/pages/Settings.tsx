...
import { Lock, Mail, LogOut, User, Save, AlertCircle, CheckCircle, Moon, Sun, Palette, Settings as SettingsIcon, Clock } from 'lucide-react';
...
export default function Settings() {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'security' | 'appearance' | 'app_settings'>('profile');
            ...
            {activeTab === 'app_settings' && (
              <form onSubmit={handleUpdateSettings} className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">بداية الدوام الرسمي</label>
                      <TimePicker
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">نهاية الدوام الرسمي</label>
                      <TimePicker
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700/30 p-4 rounded-xl border border-gray-100 dark:border-gray-700 space-y-4">
                    <div className="flex items-center justify-between">
                      <label className="text-sm font-medium text-gray-700 dark:text-gray-300">يوجد استراحة غداء</label>
                      <button
                        type="button"
                        onClick={() => setSettings({...settings, has_break: !settings.has_break})}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-durra-green focus:ring-offset-2 ${
                          settings.has_break ? 'bg-durra-green' : 'bg-gray-200 dark:bg-gray-600'
                        }`}
                      >
                        <span
                          className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                            settings.has_break ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
                    ...
                    {settings.has_break && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">بداية وقت الاستراحة</label>
                          <TimePicker
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">نهاية وقت الاستراحة</label>
                          <TimePicker
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">نسبة بونص العمل الإضافي (Multiplier)</label>
                    <input
                      type="number"
                      step="0.05"
                      min="1"
