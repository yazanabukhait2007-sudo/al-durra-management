...
try {
    db.exec("ALTER TABLE workers ADD COLUMN national_id TEXT");
}
catch (e) { }
try {
    db.exec("ALTER TABLE workers ADD COLUMN age INTEGER");
}
catch (e) { }
try {
    db.exec("ALTER TABLE workers ADD COLUMN notes TEXT");
}
catch (e) { }
try {
    db.exec("ALTER TABLE workers ADD COLUMN social_security_amount REAL DEFAULT 0");
}
catch (e) { }
try {
...
try {
    db.exec("ALTER TABLE users ADD COLUMN worker_id INTEGER");
}
catch (e) { }
try {
    db.exec("ALTER TABLE users ADD COLUMN last_password_update TEXT");
}
catch (e) { }
try {
    db.exec("ALTER TABLE task_entries ADD COLUMN target_quantity INTEGER");
}
catch (e) { }
try {
    db.exec("ALTER TABLE pallets ADD COLUMN packaging_certificate_data TEXT");
}
catch (e) { }
...
function applyMonthlySocialSecurity() {
    try {
        ...
        const workers = db.prepare("SELECT id, social_security_amount FROM workers WHERE social_security_amount > 0").all();
        const transaction = db.transaction(() => {
          ...
          `).run(worker.id, worker.social_security_amount, firstOfMonth, description);
            ...
            const job = db.prepare("SELECT last_run_date FROM system_jobs WHERE job_name = 'social_security_deduction'").get();
            if (!job) {
...
if (!adminExists) {
    db.prepare("INSERT INTO users (username, email, password, status, role, permissions) VALUES (?, ?, ?, ?, ?, ?)").run("admin", "admin@example.com", hashedAdmin, "approved", "admin", JSON.stringify(["manage_workers", "manage_tasks", "add_evaluation", "edit_evaluation", "delete_evaluation", "view_evaluations", "view_reports", "manage_users", "view_audit_logs"]));
}
else {
...
const logAction = (req, action, entityType, entityId, details) => {
    try {
        ...
        db.prepare("INSERT INTO audit_logs (user_id, username, action, entity_type, entity_id, details) VALUES (?, ?, ?, ?, ?, ?)").run(userId, username, action, entityType, entityId, details);
    }
    catch (e) {
...
try {
    const evals = db.prepare("SELECT id FROM daily_evaluations").all();
    ...
    const migrateTransaction = db.transaction(() => {
        for (const ev of evals) {
            const entries = getEntries.all(ev.id);
            let totalScore = 0;
            for (const entry of entries) {
                const task = getTask.get(entry.task_id);
...
async function startServer() {
    ...
    app.get("/api/debug/permissions/:username", (req, res) => {
        ...
        const user = db.prepare("SELECT permissions FROM users WHERE username = ?").get(username);
        if (!user) {
        ...
        try {
            ...
            const transaction = db.transaction(() => {
                if (official_start_time)
                    update.run('official_start_time', official_start_time);
                if (official_end_time)
                    update.run('official_end_time', official_end_time);
                if (break_start_time)
                    update.run('break_start_time', break_start_time);
                if (break_end_time)
                    update.run('break_end_time', break_end_time);
                if (has_break !== undefined)
                    update.run('has_break', has_break ? '1' : '0');
                if (overtime_rate)
                    update.run('overtime_rate', overtime_rate.toString());
            });
            transaction();
    ...
    app.post("/api/auth/signup", (req, res) => {
        ...
        try {
            ...
            const token = jwt.sign({ id: user.id, username: user.username, role: user.role, permissions: user.permissions }, JWT_SECRET, { expiresIn: "24h" });
            res.json({
    ...
    app.get("/api/auth/me", authenticateToken, (req, res) => {
        const user = db.prepare("SELECT id, username, email, role, permissions, status, worker_id FROM users WHERE id = ?").get(req.user.id);
        if (!user)
            return res.status(404).json({ error: "User not found" });
        res.json({
            id: user.id,
    ...
    app.put("/api/production/pallets/:id", authenticateToken, (req, res) => {
        ...
        try {
            console.log(`DEBUG: Updating pallet ${id}, user role: ${req.user.role}, body: ${JSON.stringify(req.body)}`);
            const pallet = db.prepare("SELECT * FROM pallets WHERE id = ?").get(id);
            if (!pallet)
                return res.status(404).json({ error: "Pallet not found" });
            // Check permissions for general editing
            const canEdit = req.user.role === 'admin' ||
                (req.user.permissions && req.user.permissions.includes('edit_production'));
    ...
    app.post("/api/warehouse/transfer", authenticateToken, (req, res) => {
        ...
        if (!pallet_id || !location)
            return res.status(400).json({ error: "Pallet ID and location are required" });
        try {
        ...
        try {
            const transaction = db.transaction(() => {
                for (const pallet_id of pallet_ids) {
                    ...
                    db.prepare("INSERT INTO shipping_logs (pallet_id, shipping_date, destination, driver_name, truck_number, notes) VALUES (?, ?, ?, ?, ?, ?)").run(pallet_id, shipping_details?.shipping_date, shipping_details?.destination, shipping_details?.driver_name, shipping_details?.truck_number, shipping_details?.notes);
                }
            });
            transaction();
    ...
    app.get("/api/warehouse/shipments", authenticateToken, (req, res) => {
      ...
      `).all();
            ...
            logs.forEach((log) => {
                ...
                let pkgCert = null;
                try {
                    prodCert = log.production_certificate_data ? JSON.parse(log.production_certificate_data) : null;
                }
                catch (e) { }
                try {
                    pkgCert = log.packaging_certificate_data ? JSON.parse(log.packaging_certificate_data) : null;
                }
                catch (e) { }
    ...
    app.post("/api/warehouse/requests", authenticateToken, (req, res) => {
        ...
        if (!pallet_id)
            return res.status(400).json({ error: "Pallet ID is required" });
        try {
    ...
    app.get("/api/packaging/incoming", authenticateToken, (req, res) => {
        try {
            ...
            const readyPallets = pallets.filter((p) => {
                try {
    ...
    app.post("/api/workers", authenticateToken, requirePermission("add_worker"), (req, res) => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(name, phone || null, alt_phone || null, address || null, national_id || null, age || null, last_workplace || null, current_job || null, salary || null, has_social_security ? 1 : 0, social_security_amount || 0, notes || null);
    ...
    app.put("/api/workers/:id", authenticateToken, requirePermission("edit_worker"), (req, res) => {
        ...
        if (!name)
            return res.status(400).json({ error: "Name is required" });
        try {
            const oldWorker = db.prepare("SELECT * FROM workers WHERE id = ?").get(id);
            if (oldWorker) {
                const hasChanges = oldWorker.name !== name ||
        ...
        WHERE id = ?
      `).run(name, phone || null, alt_phone || null, address || null, national_id || null, age || null, last_workplace || null, current_job || null, salary || null, has_social_security ? 1 : 0, social_security_amount || 0, notes || null, id);
    ...
    app.delete("/api/workers/:id", authenticateToken, requirePermission("delete_worker"), (req, res) => {
        ...
        const transaction = db.transaction(() => {
            ...
            const evals = db.prepare("SELECT id FROM daily_evaluations WHERE worker_id = ?").all(id);
    ...
    app.post("/api/tasks", authenticateToken, requirePermission("add_task"), (req, res) => {
        ...
        if (!name || !target_quantity)
            return res.status(400).json({ error: "Name and target_quantity are required" });
        try {
            // Check if task exists (including inactive ones)
            const existingTask = db.prepare("SELECT * FROM tasks WHERE name = ?").get(name);
            if (existingTask) {
    ...
    app.post("/api/evaluations", authenticateToken, requirePermission("add_evaluation"), (req, res) => {
        ...
        for (const entry of entries) {
            ...
            if (!task)
                return res.status(400).json({ error: `Task ID ${entry.task_id} not found` });
    ...
    app.post("/api/attendance", authenticateToken, requirePermission("manage_attendance"), (req, res) => {
        ...
        try {
            const settings = getSettings();
            const worker = db.prepare("SELECT salary FROM workers WHERE id = ?").get(worker_id);
            const transaction = db.transaction(() => {
          ...
          `).run(worker_id, date, status, check_in || null, check_out || null, finalNotes);
                ...
                if (overtimeAmount > 0) {
                    ...
                    const existingTrans = db.prepare("SELECT id FROM worker_transactions WHERE worker_id = ? AND date = ? AND type = 'bonus' AND description LIKE 'عمل إضافي%'").get(worker_id, date);
                    if (existingTrans) {
            ...
            `).run(worker_id, overtimeAmount, date, desc);
                ...
                const existingEarlyTrans = db.prepare("SELECT id FROM worker_transactions WHERE worker_id = ? AND date = ? AND type = 'deduction' AND description LIKE 'خصم خروج مبكر%'").get(worker_id, date);
                ...
                const existingLateTrans = db.prepare("SELECT id FROM worker_transactions WHERE worker_id = ? AND date = ? AND type = 'deduction' AND description LIKE 'خصم تأخير%'").get(worker_id, date);
    ...
    app.post("/api/departures", authenticateToken, requirePermission("manage_attendance"), (req, res) => {
        ...
        if (!worker_id || !date || !type || !start_time)
      ...
      `).run(worker_id, date, type, start_time, end_time || null, notes || null);
            ...
            if (start_time && end_time) {
                ...
                const existingAttendance = db.prepare("SELECT id, notes FROM attendance WHERE worker_id = ? AND date = ?").get(worker_id, date);
    ...
    app.get("/api/admin/worker-account/:workerId", authenticateToken, requirePermission("manage_users"), (req, res) => {
        ...
        try {
            ...
            if (!user)
                return res.status(404).json({ error: "No account found for this worker" });
            res.json(user);
        }
        catch (err) {
    ...
    app.post("/api/admin/create-worker-account", authenticateToken, requirePermission("manage_users"), (req, res) => {
        ...
        try {
            ...
            if (existingUser)
                return res.status(400).json({ error: "User account already exists for this worker" });
    ...
    app.get("/api/worker/my-stats", authenticateToken, (req, res) => {
        const workerId = req.user.worker_id;
        if (!workerId)
            return res.status(403).json({ error: "Not a worker account" });
